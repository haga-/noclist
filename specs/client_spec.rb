...
describe Client do
  ...
  describe '#get_users' do
    ...
    let(:token) { '67723F81-4206-BE4C-308C-2F749101B124' }
    let(:checksum) { Digest::SHA256.hexdigest(token + '/users') }

    context 'when auth token is correct' do
      context "and API doesn't raise any errors" do
      ...
      context "and API raises one error but succeeds on the next retry" do
        it 'sends a request with the correct headers and returns the response body' do
          stub_request(:get, 'localhost:8888/users')
            .with(headers: {'X-Request-Checksum' => checksum })
            .to_return(
              { status: 500, body: 'Internal server error' },
              { status: 200, body: users_list }
            )
      ...
      context "and API raises two errors but succeeds on the next retry" do
        it 'sends a request with the correct headers and returns the response body' do
          stub_request(:get, 'localhost:8888/users')
            .with(headers: {'X-Request-Checksum' => checksum })
            .to_return(
      ...
      context "and API raises one error but succeeds on the next retry" do
        it 'sends a request with the correct headers and returns the response body' do
          ...
          expect { client.get_users(token) }.to raise_error(Client::Error)
        end
      end
    end

    context 'when auth token is not correct' do
      let(:token) { 'bad_token' }
      ...
      it 'raises an error' do
        stub_request(:get, 'localhost:8888/users')
          .with(headers: {'X-Request-Checksum' => checksum })
          .to_return(
            { status: 500, body: "Invalid checksum, expected 541A80E58C9531EE00DF73851E60E6FA4148A1C85DFA774C5E3F3D1D1D4858D6 but got #{checksum}\n" },
            { status: 500, body: "Invalid checksum, expected 541A80E58C9531EE00DF73851E60E6FA4148A1C85DFA774C5E3F3D1D1D4858D6 but got #{checksum}\n" },
            { status: 500, body: "Invalid checksum, expected 541A80E58C9531EE00DF73851E60E6FA4148A1C85DFA774C5E3F3D1D1D4858D6 but got #{checksum}\n" }
          )
